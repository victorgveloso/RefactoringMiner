...
class TemporaryRemoteTest {
    ...
    void testClosePreconditions(String remoteUri, @TempDir Path tempDir) throws Exception {
        Path tmp = Files.createDirectory(tempDir.resolve("tmp"));
        Path local = Files.createDirectory(tempDir.resolve("local"));
        Git tmpGit = Git.cloneRepository().setBare(true).setDirectory(tmp.toFile()).setURI(remoteUri).call();
        Git localGit = Git.cloneRepository().setBare(true).setDirectory(local.toFile()).setURI(tmpGit.getRepository().getDirectory().getPath()).call();
    ...
    @ParameterizedTest
    @MethodSource
    void testClose(String remoteUri, @TempDir Path tempDir) throws Exception {
        Path tmp = Files.createDirectory(tempDir.resolve("tmp"));
        Path local = Files.createDirectory(tempDir.resolve("local"));
        Git tmpGit = Git.cloneRepository().setBare(true).setDirectory(tmp.toFile()).setURI(remoteUri).call();
        Git localGit = Git.cloneRepository().setBare(true).setDirectory(local.toFile()).setURI(tmpGit.getRepository().getDirectory().getPath()).call();
    ...
    @ParameterizedTest
    @MethodSource("testClose")
    void testMockedClose(String remoteUri, @TempDir Path tempDir) throws Exception {
        ...
        Git localGit = Git.cloneRepository().setBare(true).setDirectory(tempDir.toFile()).setURI(remoteUri).call();
    ...
    @ParameterizedTest
    @MethodSource("testClose")
    void testConstructor(String remoteUri, @TempDir Path tempDir) throws Exception {
        Path local = Files.createDirectory(tempDir.resolve("local"));
        Git localGit = Git.cloneRepository().setBare(true).setDirectory(local.toFile()).setURI(remoteUri).call();
        Path tmp = Files.createDirectory(tempDir.resolve("tmp"));
        ...
        assertEquals(remoteUri, temporaryRemote.getTempGit().remoteList().call().get(0).getURIs().get(0).toString());
