...
public abstract class AbstractCodeMapping implements LeafMappingProvider {
	...
	public void temporaryVariableAssignment(AbstractCodeFragment statement,
		...
		for(VariableDeclaration declaration : statement.getVariableDeclarations()) {
			...
			for(Replacement replacement : replacements) {
				...
				boolean comprehensionMatch = false;
				if(after.contains(variableName) && initializer != null) {
					ComprehensionExpression comprehension = initializer.comprehensionCoveringEntireFragment();
					if(comprehension != null && replacement.getType().equals(ReplacementType.COMPREHENSION_CLAUSE)) {
						if(comprehension.getString().contains(before)) {
							comprehensionMatch = true;
						}
					}
				}
	...
	private static boolean involvesMethodInvocation(Replacement replacement) {
		return replacement instanceof MethodInvocationReplacement ||
				...
				replacement.getType().equals(ReplacementType.METHOD_INVOCATION_REPLACED_WITH_INFIX) ||
